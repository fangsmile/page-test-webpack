...
@Serialize({})//序列化
export class Container extends Serializable {
    @SerializeProperty()
    ...
    //绘制该组所有内容
    // draw(isThumb:boolean=false) {
    //     this.children.forEach(obj => {
    //         obj.draw();
    //     })
    // }
    draw(ctx: CanvasContext, hitCtx: CanvasContext, isThumb: boolean = false) {
        this.children.forEach(obj => {
            obj.draw(ctx, hitCtx, isThumb);
        })
    }
    getIndex() {
        return this.index;
