import { IRichText, TextAlgin } from "../../text-editor/ITextEditor";
...
export class RichText extends Serializable implements IRichText {
    ...
    public styles: List<StyleDefine>;
    @SerializeProperty({
        type: List
    })
    public lineHeights: List<number>;
    @SerializeProperty()
    public lineHeight: number;
    @SerializeProperty({
        type: List
    })
    public lineWidths: List<number>;
    ...
    renderCache: List<List<TextStyleInfo>>;
    @SerializeProperty()
    config: TextConfig;
    ...
    isDrawHit: boolean = true;
    isSelected: boolean = false;
    currentScale: number;

    keyboardImgElem:any;
    hasLoadKeyboardImg: boolean = false; //键盘png是否已加载，只加载一次，后续直接用

    constructor(config: TextConfig, fontSize: number, changeToEditModeImg: HTMLImageElement, colorKey: string, fill: string = "#000000") {
        super();
        this.config = config;
        this.styles = new List<any>();
        ...
        this.lineWidths = new List<number>();
        this.lineHeights = new List<number>();
        this.minWidth = 110;
        this.maxWidth = 600;
    ...
    public updateRenderCache(ctx: CanvasContext) {
        ...
        var textStyleArr: List<List<TextStyleInfo>> = new List<List<TextStyleInfo>>();; //这个文本的所有样式
        var style = {};
        var obj = null;
        var that = this;
        ...
        for (var j = 0; j < textLines.length; j++) {
            var lineStyleArr: List<TextStyleInfo> = new List<TextStyleInfo>(); //每行对应的所有样式
    ...
    //非编辑状态下文字绘制
    public draw(ctx: CanvasContext, hitCtx: CanvasContext,isThumb:boolean=false) {
        this.currentScale = ctx.scaleVal;
        let point = isThumb?this.getCenterCoordinate():new Point(this.config.translate.x,this.config.translate.y);
        ctx.scalePoint(1 / this.currentScale, 1 / this.currentScale, point.x, point.y);
        ctx.save();
        this.drawText(ctx);
        ctx.restore();
        if (this.isEditting) {
        ...
        ctx.scalePoint(this.currentScale, this.currentScale, point.x, point.y);
        hitCtx.scalePoint(1 / this.currentScale, 1 / this.currentScale, point.x, point.y);

        if (hitCtx && this.isDrawHit) {
            hitCtx.save();
            hitCtx.beginPath();
            hitCtx.fillStyle = this.colorKey;
            hitCtx.rect(this.config.translate.x, this.config.translate.y, this.config.width, this.config.height);
            hitCtx.fill();
            hitCtx.closePath();
            hitCtx.restore();
        }
        hitCtx.scalePoint(this.currentScale, this.currentScale,point.x, point.y);
    ...
    private drawChangeModeImg(point: Point, context: CanvasContext) {
        this.changeToEditModeImg && context.drawImage(this.changeToEditModeImg, point.x, point.y);
    }
    ...
    private renderTextLine2(ctx: CanvasContext, line: string, left: number, top: number, lineIndex: number, heightOfLine: number) {
        ...
        //http://jira.xuelebj.net/browse/CLASSROOM-5754 下面代码会导致 最后一个字符为空格时，右对齐，文字 向左偏移量过大 出了输入框的范围，故注释掉；
        // if(this.textAlign === TextAlgin.right && text.slice(-1) == " "){
        //     left = left -  lineStyle.get(0).style.width;
        // }
        ...
        for (i = 0; i < arrLen; i++) {
            ...
            //TODO -- 绘制文字时要考虑最后一个是空字符  “ ” 的情况，在右对齐的情况下要左移半个字符
