...
export class XlMath {
    ...
    static pointMove2(p1: Point, p2: Point, d: number) {
        ...
        } else if(p1.y - p2.y == 0){//p1和p2为同一个点，则不进行计算；http://jira.xuelebj.net/browse/CLASSROOM-5303
     ...
     *  小数转分数
     */
    static decimalsToFractional(decimals: number) {
        var formatDecimals: any = decimals.toFixed(3);
        var denominator = 1000;//初始化分母
        var numerator = formatDecimals * 1000;//初始化分子
        var bigger = 0;
        function recursion() {
            bigger = denominator > numerator ? denominator : numerator;
            for (let i = bigger; i > 1; i--) {
                if ((<any>Number).isInteger(numerator / i) && (<any>Number).isInteger(denominator / i)) {
    ...
    static computePointAfterRotate(x: number, y: number, x0: number, y0: number, degree: number): Point {
    ...
    static rectPoint(points: Array<Point>) {
        ...
        points.forEach((element: Point) => {
            if (element.x || element.x === 0) {//添加对0的兼容
    ...
    //保留两位小数
    //功能：将浮点数四舍五入，取小数点后2位
    static toDecimal(x: number, digitNumber: number = 2) {
        // var f = parseFloat(x);
        if (isNaN(x)) {
            return;
        }
        var f = Math.round(x * Math.pow(10, digitNumber)) / Math.pow(10, digitNumber);
        return f;
    }
    /**
     * s是否接近整除 a是否是b的整数倍
    */
    static nearDivided(a: number, b: number, tolerance: number = 0) {
        let n = a % b;
        if (Math.abs(n) <= tolerance || b - Math.abs(n) <= tolerance) {
            return true;
        }
        return false;;
    }
    ...
    */
    ...
    static distance(b:any, e:any) {
        return Math.sqrt(Math.pow(e.x - b.x, 2) + Math.pow(e.y - b.y, 2));
    }
    static bezierCalculate(poss:Array<Point>, precision:number) {
